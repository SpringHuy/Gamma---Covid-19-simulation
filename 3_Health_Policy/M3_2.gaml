...
global {
	...
	float asym_infected_prob <- 0.093;
	...
	int nb_Asymptomatic_Individuals update: Individuals count each.is_Asymptomatic;
...
species LocalAuthority {
	...
	//for M3_2 - this is most 
		...
		//this testing is to detect if a person should be qurantine or not, not for drop that quarantine
	reflex testing when: P1.p2_start_test and current_hour = 10 and nb_Infectious_Individuals > 2 {	
		P1.p2_start_test <- true;
			loop i from: 0 to: length(Individuals) - 1 {
				if(!Individuals[i].is_quarantine and flip(P1.test_rate)){
					if(Individuals[i].is_Infectious and flip(0.89) ){
						Individuals[i].is_quarantine <- true;					
					} else if((Individuals[i].is_Susceptible or Individuals[i].is_Exposed) and flip(0.18)){
						Individuals[i].is_quarantine <- true;						
					}					
				}
			}			 
		
		//Quarantine should be the central counter measure action, even after three tests in series return negative, that person should still be self-quarantined at home
		//Quanrantine at home is also the most possible whe hospitals are overloaded 
		//Increase test rate if nb of positive increase,		

	}

	reflex drop_testing when: P1.p2_start_test and nb_Infectious_Individuals = 0 and nb_Quarantine_Individuals = 0 {
		P1.p2_start_test <- false;		
	}
	...
	reflex drop_wear_mask when: P1.p4_wear_mask and nb_Infectious_Individuals = 0 and nb_Quarantine_Individuals = 0 {
		P1.p1_total_free <- true;
...
///How the quarantine will affect agenda - 
	...
	reflex go_home_quarantine when: is_quarantine and current_building != my_home { //infectious will go home after executing previous reflex, other in close contact will be quarantined also
		do go_home;
	}
	
	reflex Indi_drop_quaran when: is_quarantine and hour_s_quaran = current_hour and day_s_quaran + P1.self_isolation_period = current_day {
		//triple tests to decide whether to drop quarantine on the person
			if ((is_Susceptible or is_Exposed) and flip(0.92) and flip(0.92) and flip(0.92)) {
				is_quarantine <- false;
				day_s_quaran <- nil;
				hour_s_quaran <- nil;
			}else if (is_Infectious and flip(0.11) and flip(0.11) and flip(0.11)) {
				is_quarantine <- false;
				day_s_quaran <- nil;
				hour_s_quaran <- nil;
			} 
...
experiment E3_2_test type: gui {
	...
	permanent {
		display series_Infectious_Individuals {
			chart "Individuals in SEIR - E3.2" type: series {
...
experiment E3_2 type: gui {
	...
	output {
		...
		monitor "Asymptomatic Individuals" value: nb_Asymptomatic_Individuals;
